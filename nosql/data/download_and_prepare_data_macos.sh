...
gsed -i '1s/movieId/movieId:ID(Movie)/' movies.csv

echo 'Fixing header of users.csv'
gsed -i '1s/userId/userId:ID(User)/' users.csv

echo 'Fixing header of ratings.csv'
gsed -i '1s/userId/:START_ID(User)/' ratings.csv
gsed -i '1s/movieId/:END_ID(Movie)/' ratings.csv
gsed -i '1s/rating/rating:float/' ratings.csv
gsed -i '1s/timestamp/timestamp:int/' ratings.csv
